...
app.post('/api/recordings', upload.single('video'), async (req, res) => {
  ...
  const relativeFilepath = `uploads/${filename}`; // Relative path to fix ENOENT
  try {
...
app.get('/api/recordings', async (req, res) => {
  try {
    const recordings = await Recording.find().select('filename filesize createdAt');
    const response = recordings.map(rec => ({
      id: rec._id,
      filename: rec.filename,
      filesize: rec.filesize,
      createdAt: rec.createdAt,
      url: `http://localhost:${PORT}/api/recordings/${rec._id}`,
    }));
    res.json(response);
  } catch (err) {
    console.error('DB error:', err);
